...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor(address owner_, address rewardToken_, address router_) {
        _token = msg.sender;
        owner_ = owner_;
    ...
    //My modifications to the process function
    function process(uint256 gas) external override onlyTokenAndOwner {
        ...
        uint256 index = currentIndex; //retrieving storage variable costs a lot of gas that adds up quickly in loops
        //better to store it in an memory variable, then save it back to storage afterwards.
        //Make sure that there is always at least 20000 gas left for the storing of the currentIndex back to storage.

        while (gasUsed + 20000 < gas && iterations < shareholderCount) {
